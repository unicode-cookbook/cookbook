...
\begin{itemize}
   ...
   \item The uses of the apostrophe has led to long discussions on the Unicode
        Standard email list. An English keyboard inputs <{\fontspec{Monaco}'}>
        \textsc{apostrophe} at \uni{0027}, although the preferred Unicode
        ...
        \uni{02BC}, which unfortunately looks mostly extremely similar to
        \uni{2019}. 
  \item Another problem is the <ˁ> \textsc{modifier letter reversed glottal
        stop} at \uni{02C1} vs.\@ the <ˤ> \textsc{modifier letter small reversed
  ...
  \item The dental click <ǀ>, in Unicode known as \textsc{latin letter dental
        click} at \uni{01C0}, is of course often simply typed as <|>
        \textsc{vertical line} at \uni{007C}.
  \item For the marking of length there is a special Unicode characters, namely
        <ː> \textsc{modifier letter triangular colon} at \uni{02D0}. However,
        typing <:> \textsc{colon} at \uni{003A} is of course much easier.        
...
\begin{itemize}
   ...
   \item Similarly, the alveolar lateral click <ǁ>	should be encoded with a
           ...
           encoded by \textsc{double vertical line} at \uni{2016}.
...
tone, and <\ ́~> \textsc{combining acute tone mark} at \unif{0341} vs.\@ <\ ́~>
...
Third, the IPA has different markers for velarization <ˠ> and pharyngealization
<ˤ>. Confusingly, there is also a marker for ``velarized or pharyngealized'',
using the <\ \ {\large ̴}> \textsc{combining tilde overlay} at \uni{0334}. The
...
e.g. <\charis{t͡s}> or <\charis{t͜s}>. IPA allows both options. The choice
...
\textsc{combining double inverted breve} at \uni{0361}.
...
%	  \item[→] frictionalized <ɾ͓>
	  \item[→] more rounded <ɔ̹> \textbar{} less rounded <ɔ̜>
	  \item[→] creaky voiced <b̰> \textbar{} breathy voiced <b̤> \textbar{} voiced <s̬> \textbar{} voiceless <n̥>
...
%	  \item[→] (tone accents, e.g. <è>)
	  \item[→] no audible release <p̚>
%	  \item[→] Spacing Modifier Letters, see above	  
 \end{itemize} \end{itemize}

Then, when a character sequence contains more than one character in Spacing
...
\input{chapters/tab:unicode_letters}

\input{chapters/tab:unicode_diacritics} 
