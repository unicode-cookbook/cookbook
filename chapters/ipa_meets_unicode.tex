...
are also included, see~\cite[Appendix~C]{Moran2012}. The 157 characters from 12 
...
like <\dia{0334}>, always come before diacritics below, which in turn always
come before diacritics above. We follow this order and add the other IPA
diacritics (which are not diacritics in the Unicode sense) after these
diacritics. Within these classes of diacritics there is no canonical ordering
specified by Unicode, so we propose an explicit ordering here.
...
157 different unicode codepoints that form the basis of strict-IPA encoding
(107 letters, 43 diacritics and 7 remaining symbols). Each of
...
\begin{itemize}[itemsep=6pt]
  ...
  \item \textsc{IPA diacritics} \newline The 43 different IPA diacritics and
