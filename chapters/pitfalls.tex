...
<{\fontspec{EversonMono}AАΑᎪᗅᴀꓮ𐊠𐌀𝖠𝙰}> are all different code points,
...
\unif{0063}, \unif{0065} and \unif{0073}, while the second is a mix of Cyrillic and
Greek characters, namely \unif{03BD}, \unif{03BF}, \unif{0041}, \unif{0435} and
\unif{0455}.} For computers these two words are completely different entities.
...
\textsc{angstrom sign} (\uni{212B}), instances of \textsc{latin capital letter a
with ring above} (\uni{00C5}) would not be found.
...
i.e.~\textsc{latin small letter a} at \uni{0061} and \textsc{combining grave
accent} at \uni{0300}, this might be converted into <à>, i.e.~\textsc{latin
small letter a with grave} at \uni{00E0}.\footnote{The behavior of software
...
\uni{00E7}, second the ogonek, like in <ą> \textsc{latin small letter a with
ogonek} at \uni{0105} and, third, the horn, like in <ơ> \textsc{latin small
letter o with horn} at \uni{01A1}. These characters are indeed decomposed in a 
...
<\ \ {\large  ̴}> \textsc{combining tilde overlay} at \uni{0334} or 
<\ \ {\large  ̵}> \textsc{combining short stroke overlay} at \uni{0335}. 
There are many characters that look like they are 
precomposed with such an overlay, for example 
<{\small \fontspec{CharisSIL}ɫ ᵬ ᵭ ᵱ}> or 
<{\small \fontspec{CharisSIL}ƚ ɨ ɉ ɍ}>, or also the 
...
\textsc{combining palatalized hook below} at \uni{0321}. This element seems to
occur in precomposed characters like <{\small \fontspec{CharisSIL}ᶀ ᶁ ᶂ ᶄ}>. 
However, all these characters are not canonical equivalent with a combination of 
their base characters with the combining overlay or combining hook.
...
issue with code units stems from the question how to separate a stream of binary
ones and zero, i.e.~bits, into chunks representing Unicode characters. Depending
on different use cases, the Unicode Standard offers three different approaches,
called UTF-32, UTF-16 and UTF-8.\footnote{The letters UTF stand for either
\textsc{Unicode Transformation Format} or \textsc{Universal Coded Character Set
Transformation Format}, but the notion of \textit{transformation} is a legacy
notion that does not have meaning anymore. Nevertheless, the designation
\textsc{UTF} (in capitals) has become an official standard designation, but
should probably best be read as simply \textit{Unicode Format}.} The details of
this issue is extensively explained in section 2.5 of the Unicode core
specification. Basically, \textsc{UTF-32} encodes each character in 32 bits (32
\textit{bi}nary un\textit{ts}, i.e.~32 zeros or ones) and is the most
disk-space-consuming variant of the three. However, it is the most efficient
encoding processing-wise, because the computer simply has to separate each
character after 32 bits. In contrast, \textsc{UTF-16} uses only 16 bits per
character, which is sufficient for the large majority of Unicode characters, but
not for all of them. A special system of `surrogates' is defined within the
Unicode Standard to deal with these additional characters. The effect is a more
disk-space efficient encoding (approximately half the size), while adding a
limited computational overhead. Finally, \textsc{UTF-8} is a more complex system
that dynamically encodes each character with the minimally necessary number of
bits, choosing either 8, 16 or 32 bits depending on the character. This
represents again a strong reduction in space (particularly due to the high
frequency of data using ASCII characters that need only 8 bits) at the expense
of even more computation necessary to process such strings. However, because of
the ever growing computational power of modern machines, the processing overhead
is in most practical situations a non-issue, while saving on space is still
useful, particularly for sending texts over the Internet. As an effect, UTF-8
has become the dominant encoding on the World Wide Web. We suggest that
everybody uses UTF-8 as their default encoding.
...
beyond the scope of this paper. It suffices to realize that there is a
...
what kind of storage is used by starting a file with a so-called `Byte Order
...
endianness is used. This issue only arises with UTF-32 and UTF-16 encodings, and
...
Together with the topic of normalization (NFC vs.~NFD, see Pitfall 5 above),
this leads to the following nicely cryptic file-encoding recommendation, namely
to preferably use \textsc{UTF-8 in NFC without BOM with LF line breaks},
whenever possible.