...
\section{Overview}
To illustrate the practical applications of orthography profiles, as defined in Chapter~\ref{orthography-profiles}, we have implemented two versions of the specifications presented there: one in Python\footnote{\url{https://pypi.python.org/pypi/segments}} and one in R.\footnote{\url{https://github.com/cysouw/qlcData}} This chapter introduces these two implementations, and gives practical step-by-step guidelines for installing and using them. Various simple and sometimes somewhat abstract examples will be discussed to show the different options available, and to illustrate the intended usage of orthography profiles. 

Note that our two software libraries have rather different implementation histories, thus they may not give the same results in all situations. We do, however, provide a test suite for each implementation aimed at making sure that the results covered by those test cases are identical. Users should refer to the tests and to the documentation in each release for specifics regarding each implementation. Due to the different naming conventions in Python and R, function names differ across the two implementations. The performance with larger datasets also differs per implementation. In sum, the two software libraries should be considered as proof of concept and not as the final word on the practical application of the specifications above. In our own experience, the current versions are sufficiently fast and stable to be useful for academic practice (e.g.\ checking data consistency, or analyzing and transliterating small to medium sized data sets), but they should probably not be used for full-scale industry applications without some adaptations.

In Section \ref{installing-python-and-r}, we first explain how to install Python\footnote{\url{https://www.python.org/}} and R.\footnote{\url{https://www.r-project.org/}} In Sections \ref{python-implementations} and \ref{r-implementation}, we discuss our Python and R software packages, respectively. In addition to the material presented here to get users started, we maintain several case studies that use orthography profiles in action. For convenience, we make these use cases available as Jupyter Notebooks\footnote{\url{http://jupyter.org/}} in our Github repository of code recipes, which accompanies this book online.\footnote{\url{https://github.com/unicode-cookbook/}} In the final section in this chapter, we briefly describe these additional use-cases and point the user to the accompanying code online.
...
You can also install the \texttt{segments} package with from the Github repository:
...
The \texttt{segments} API can be accessed by importing the package into Python or by writing a Python script. Here w\is an example of how to import the libraries, create a tokenizer object, tokenize a string, and create an orthography profile.
...
% \begin{lstlisting}[language=python]
\begin{lstlisting}[inputencoding=utf8,extendedchars=true,escapeinside={\%*}{*)},texcl=true]
>>> t = Tokenizer()
...
\noindent The \texttt{grapheme\_clusters} function is also the default segmentation algorithm for the \texttt{segments.Tokenizer}. It is useful when you encounter a text that you want to tokenize to identify orthographic or transcription elements.
...
\noindent The \texttt{ipa} parameter forces grapheme segmentation for IPA strings (a formal definition is given in Chapter \ref{orthography-profiles}.
...
\noindent We can also load an orthography profile and tokenize input string with it. In the data directory we've placed an example orthography profile. Let's have a look at it using \texttt{more} on the command line.
...
\noindent This example shows how can we tokenize input text into our orthographic specification. We can also segment graphemes and transliterate them into other formats, which is useful when you have sources with different orthographies, but you want to be able to compare them using a single representation like IPA.
...
\noindent We set the default as the Unicode replacement character \uni{fffd}, \textsc{replacement character}.\footnote{\url{http://www.fileformat.info/info/unicode/char/fffd/index.htm}} But you can simply change this by specifying the replacement character when you load the orthography profile with the tokenizer.
...
\noindent Perhaps you want to create an initial orthography profile that also contains those graphemes x, y, z?
...
(called user-perceived characters, see Chapter~\ref{the-unicode-approach}). As it 
...
Online we provide several recipes, or use cases, that illustrate the applications of orthography profiles using our implementations in Python and R. \footnote{\url{https://github.com/unicode-cookbook/recipes}} Here we briefly describe these use cases and we encourage users to try them out using Git and Jupyter Notebooks.

First, as we discussed above, we provide a basic tutorial on how to use the Python \texttt{segments}\footnote{\url{https://pypi.python.org/pypi/segments}} and R \texttt{qlcData}\footnote{\url{https://github.com/cysouw/qlcData}} libraries.\footnote{\url{https://github.com/unicode-cookbook/recipes/tree/master/Basics}} This use case simply shows the basic functions of each library.

The two use cases using the Python \textt{segments} package include a tutorial on how to segment graphemes in IPA text:
...
\noindent The JIPA use case uses excerpts from the Northwind and the Sun passages from the Illustrations of the IPA published in the Journal of the International Phonetic Alphabet. Thus the use case shows how a user might tokenize IPA proper. The Dogon use case uses fieldwork data from the Dogon languages of Mali language documentation project.\footnote{\url{http://dogonlanguages.org/}} This use case illustrates how a user might tokenize fieldwork data from numerous linguists using different transcription practices by defining these practices with an orthography profile to make the output unified and comparable.

The two use cases using the R \texttt{qlcData} library include a use case for tokenizing wordlist data from the Automated Similarity Judgment Program (ASJP):\footnote{\url{http://asjp.clld.org/}}
...
\noindent The ASJP use case shows how to download the full set of ASJP wordlists, to combine them into a single large CSV file, and to tokenize the ASJP orthography. The Dutch use case uses the 10K corpus for Dutch (``nld'') from the Leipzig Corpora Collection,\footnote{\url{http://corpora2.informatik.uni-leipzig.de/download.html}} which is then cleaned and tokenized with an orthography profile that captures the intricacies of Dutch orthography.
