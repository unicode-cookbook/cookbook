...
\noindent The version available through CRAN is the latest stable version.
...
The first example of how to use these functions considers finding errors in the
...
\noindent The function of error-message reporting can also nicely be illustrated
...
turns out, ome of these graphemes are single codepoints, others are combinations
...
\noindent By specifying the splitting separator as the empty string
...
\noindent It is also possible to leave out the Unicode information in
...
\noindent This will lead to the following four files being written. Crucially, a
...
skeleton to specify transliteration. The name of the columne
...
strings. The first is called \texttt{method = "global"} here: this approach
...
parsing. The other approach is called \texttt{method = "linear"} here: this
...
such rewrite rules, see Section~\ref{rule-ordering}).

In some special cases these two methods can lead to different results, but these
special situations are very unlikely to happen in natural language. The example
below shows that a string \texttt{'abc'} can be parsed differently in case of a
very special profile with a very special ordering of the graphemes.
...
The current R-implementation has a limitation when regular expressions are used. 
The problem is that overlapping matches are not captured. Everything works as 
expected without regular expressions, but there might be warnings/errors in case of 
\texttt{regex = TRUE}. However, just as in the previous issue, this should very 
rarely happen in natural language.
