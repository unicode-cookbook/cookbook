...
\texsct{international phonetic alphabet} (henceforth \textit{IPA}), which is a
...
a chart and were given phonetic feature labels (e.g.\ for manner of consonant
...
From its earliest days the Association aimed to provide ``a separate sign for
...
analysis so that it is widely understood. 

Basically, the IPA allows for both a `narrow' phonetic transcription and a
...
computational representations for the IPA -- the principles above of which have
had several ramifications that make the interoperability of electronic
