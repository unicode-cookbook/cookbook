...
systems. However, a common complaint from users is that they encounter symbols from a 
fall back font (e.g.\ a rectangle <▯>, question mark <?>, the Unicode replacement character <�>) 
or nothing is displayed at all. The problem is that the user's computer does not have any fonts 
installed that map Unicode code points to glyphs in the document they are trying to inspect. 
It is important to realize that internally in the computer everything
still works as expected: any handling of Unicode code points works independently
of how they are displayed on the screen. So although users might only see
alien faces being displayed, they should be assured that everything is still 
technically in order.

There are two main issues regarding missing glyphs. One issue is practical. 
Designing glyphs includes many different considerations and 
it is a time-consuming process when done well. 
Many traditional expectations of what specific characters should look like need
...
designer to try and conform to them as well as possible. Therefore, most
designers produce fonts that only include glyphs for certain parts of the Unicode
Standard. The second issue is technical. The current version of the Unicode Standard 
contains 137,374 characters, but the OpenType specification (ISO/IEC 14496-22:2015), 
which is used on all major computing platforms, is 16 bits. This means the maximal number of 
glyphs in any font is limited to 65,536.
...
\url{http://www.evertype.com/emono/}.} Somewhat older is the \textsc{Titus
...
at \url{http://titus.fkidg1.uni-frankfurt.de/unicode/tituut.asp}.} For maximum 
coverage, there is the Noto font family, a project developed by Google, 
which covers 93 scripts and around 64,000 characters.\footnote{\url{https://www.google.com/get/noto/}}

Further, we suggest installing at least one so-called \textsc{fall-back
...
Font}, currently available up to Unicode version 6.1. This font does not show a real 
...
There is no real solution to problems arising from faulty font rendering.
