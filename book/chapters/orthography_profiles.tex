...
tab-separated text file, that characterizes and documents a writing system.
...
Given these goals, Unicode locale descriptions (see
...
(ii) locale descriptions are designed for a wide scope on information (like date
...
then it is nigh impossible to deploy the description. This is because a locale
...
clusters are needed (possibly specified in an orthography profile). For example,
...
rules, e.g.~when tokenizing graphemes, in say English, the sequences <sh> in the
...
Unicode approach including the \textsc{zero-width non-joiner} character at
...
Section~\ref{r-implementation} on various exemplary use-cases). The result of any such
...
\begin{enumerate}
	...
	\item \textsc{A profile is a unicode UTF-8 encoded text file} 
	   ...
	   no-BOM and LF line breaks because of the pitfalls they avoid. 
	   A keen reviewer notes, however, that specifying 
...
%       includes the information pertinent to the orthography. 
	\item \textsc{A profile is a CSV file with an obligatory header
       line}. A minimal profile must have a single column with the header ``Grapheme''. 
	   For any additional columns, the name in the header of the CSV file
       must be specified. The actual ordering of the columns is unimportant. Cells should 
	   not contain tabs or newlines. The header list must be delimited 
...
\begin{comment}	   
	\item \textsc{Metadata should be added in a separate UTF-8 text file} using a basic
...
\end{comment}
	...
	\item \textsc{Metadata should be added in a separate UTF-8 text file} using 
	   the JSON-DL dialect specified in \textit{Metadata Vocabulary for Tabular 
	   ...
	   in the orthography profile. The orthography profile metadata should minimally include: 
	   (i) author, (ii) date, (iii) title of the profile, (iv) a stable language identifier
       encoded in BCP 47/ISO 639-3 of the target language of the profile, and (v)
       bibliographic data for resource(s) that illustrate the orthography
       described in the profile. Further, (vi) the tokenization method and (vii)
       the Unicode normalization form used should be documented here (see below).
	   In accordance with the \textit{Metadata Vocabulary for Tabular 
	   Data}, the metadata's filename must consist of the orthography 
	   profile's filename appended with ``-metadata.json''.\footnote{JSON-DL metadata 
