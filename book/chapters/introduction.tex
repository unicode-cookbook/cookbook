...
standard. A good appreciation for the solutions that the Unicode Standard
provides, also allows for a thorough understanding of possible pitfalls that one
might encounter when using it (Chapter~\ref{unicode-pitfalls}). As an example of
the current state-of-the-art, we discuss the International Phonetic Alphabet
(IPA, Chapter~\ref{the-international-phonetic-alphabet}) and its rather
problematic marriage with the Unicode Standard (Chapter
\ref{ipa-meets-unicode}).

The second part of the book describes our proposals for how to deal with the
Unicode Standard in the daily practice of linguistics. First, we discuss the
...
descriptions (Chapter~\ref{orthography-profiles}). We provide reference
